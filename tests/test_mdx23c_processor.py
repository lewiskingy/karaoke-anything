...
def packet(sequence: int, value: float = 1.0, sample_rate: int = 100) -> MediaPacket:
    ...
    header[8:12]=sample_rate.to_bytes(4,'big'); header[12:16]=sequence.to_bytes(4,'big'); header[24:26]=(25).to_bytes(2,'big')
...
async def test_process_rejects_non_stereo_input() -> None:
    ...
    bad = MediaPacket.received(bytes(header) + array("f", [0.0]).tobytes(), "127.0.0.1", 1)
